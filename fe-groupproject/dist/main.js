...
/***/ "./src sync recursive ^.*\\/node_modules\\/electron$":
/*!************************************************!*\
  !*** ./src/ sync ^.*\/node_modules\/electron$ ***!
  \************************************************/
/***/ ((module) => {
...
webpackEmptyContext.id = "./src sync recursive ^.*\\/node_modules\\/electron$";
...
if (true) {
  try {
    __webpack_require__(/*! electron-reload */ "electron-reload")(__dirname, {
      electron: __webpack_require__("./src sync recursive ^.*\\/node_modules\\/electron$")(`${__dirname}/node_modules/electron`)
    });
  } catch (err) {
...
const createWindow = () => {
  ...
  mainWindow.loadURL('http://localhost:3000/main_window').then(() => console.log('Main window loaded')).catch(error => console.error('Error loading window:', error));
...
const startPythonProcess = () => {
  const pythonScriptPath = path.join(__dirname, '..', '..', 'demo v2 (cnn + mediapipe)', 'App_v1', 'main.py');
